...
export function PaymentsPage() {
  ...
  return <>
    ...
    <section className="overflow-hidden rounded-2xl bg-white">
      <div className="flex flex-wrap items-center justify-between gap-3 p-4"><div className="flex flex-wrap gap-1">{["All","Needs review","Approved","Unpaid","Rejected"].map(item => <button key={item} aria-pressed={filter===item} onClick={() => setFilter(item)} className={`rounded-lg px-3 py-2 text-[12px] font-medium ${filter===item ? "bg-accent text-primary" : "text-muted-foreground hover:bg-muted"}`}>{item}</button>)}</div><div className="relative min-w-[240px]"><IconSearch size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={e=>setQuery(e.target.value)} aria-label="Search payments" placeholder="Customer or booking code" className="h-10 w-full rounded-xl border border-transparent bg-muted pl-9 pr-3 text-[13px] text-foreground placeholder:text-muted-foreground shadow-none transition focus-visible:border-border focus-visible:bg-white focus-visible:ring-1 focus-visible:ring-primary" /></div></div>
      <div className="overflow-x-auto"><table className="w-full min-w-[700px] text-left text-[12px]"><thead className="border-b border-border bg-muted text-foreground"><tr>{["Customer / booking","Amount","Method","Status",""].map((heading,i)=><th key={i} scope="col" className="px-5 py-3.5 font-semibold text-foreground">{heading || <span className="sr-only">Actions</span>}</th>)}</tr></thead><tbody>{rows.map(({booking,payment}) => {
        const customer=state.customers.find(c=>c.id===booking.customerId);
