...
import type { Booking } from "@/lib/model";
...
export function ReservApp() {
  ...
  const onNew = (preset: BookingPreset = {}) => setNewBooking(preset);
  if (pathname === `/b/${state.business.slug}`) return <PublicProfile />;
  if (pathname === "/reservation" || pathname.startsWith("/r/"))
    return (
      <ReservationLookup
        key={pathname}
        code={pathname.startsWith("/r/") ? pathname.split("/")[2] : undefined}
      />
    );
  ...
  let page;
  if (pathname === "/")
    page = <CalendarPage onNew={onNew} onBooking={onBooking} />;
  else if (pathname === "/bookings")
    page = <BookingsPage onNew={onNew} onBooking={onBooking} />;
  else if (pathname.startsWith("/bookings/")) {
    const booking = state.bookings.find((b) => b.id === pathname.split("/")[2]);
    page = booking ? (
    ...
    page = <CustomersPage onNew={onNew} onBooking={onBooking} />;
  else if (pathname === "/services") page = <ServicesPage />;
  else if (pathname === "/business-profile")
    page = <BusinessProfilePage key={String(state.loaded)} />;
  else if (pathname === "/agent") page = <AgentPage />;
  else if (pathname === "/settings")
    page = <SettingsPage key={String(state.loaded)} />;
  else
    page = (
      <EmptyState
  ...
  return (
    <div className="min-h-screen bg-[#f4f4f5] text-[#202022]">
      <header className="relative flex h-[72px] items-center justify-between border-0 bg-[#f2f2f4] px-9 max-[760px]:h-16 max-[760px]:px-4">
        <div className="flex items-center gap-4">
          <Brand />
          <span className="h-5 w-px bg-[#d5d5d7] max-[760px]:hidden" />
          <Link
            href={`/b/${state.business.slug}`}
            className="flex items-center gap-1.5 text-[11px] font-medium text-[#6d6d70] transition hover:text-[#171719] max-[760px]:hidden"
          >
            {state.business.name}
        ...
        <div className="flex items-center gap-4">
          <span className="text-[10px] text-[#9b9b9e] max-[760px]:hidden">
            WAT · 10:15 AM
          </span>
          <button
            aria-label="New booking"
            className="flex h-8 w-8 items-center justify-center rounded-full bg-[#1e1e20] text-white transition hover:bg-[#444447]"
            onClick={() => onNew()}
          >
            <IconPlus size={19} />
          </button>
          <Link href="/settings" aria-label="Settings">
            <Avatar name={state.settings.owner} size="tiny" />
          </Link>
        </div>
      </header>
      <nav
        className="mx-auto flex h-[56px] max-w-[1440px] items-center justify-center gap-1 overflow-x-auto border-0 bg-[#f2f2f4] px-5 max-[1023px]:justify-start max-[760px]:h-12 max-[760px]:px-3"
        ...
        {navigation.map(({ name, path, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              className={`flex h-9 shrink-0 items-center gap-1.5 rounded-full px-4 text-[11px] transition max-[760px]:px-3 max-[760px]:text-[10px] ${
                active
                  ? "bg-[#fbfbfc] font-semibold text-[#202023]"
                  : "font-medium text-[#929295] hover:bg-[#e9e9eb] hover:text-[#29292b]"
              }`}
            >
      ...
      <main
        className="mx-auto max-w-[1440px] px-10 pb-16 pt-10 max-[1023px]:px-6 max-[760px]:px-4 max-[760px]:pt-6"
        key={pathname}
      >
        {page}
      </main>
      {newBooking && (
        <BookingFlow preset={newBooking} onClose={() => setNewBooking(null)} />
      )}
    </div>
  );
}
